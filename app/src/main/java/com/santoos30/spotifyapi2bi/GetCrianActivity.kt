                ...
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    for (item in snapshot.children) {
                        ...
                        btnExcluir.setOnClickListener {
                            crianca?.id?.let {
                                id -> database.getReference("criancas").child(id).removeValue()
                                mostrarMensagem("Criança removida!")
                            }
                        }
                        ...
                        btnEditar.setOnClickListener {
                            val dialogView = layoutInflater.inflate(R.layout.dialog_editar_crian, null)
                            val dialog = AlertDialog.Builder(this@GetCrianActivity).setView(dialogView).create()
                            dialog.window?.setBackgroundDrawableResource(android.R.color.transparent)
                            dialog.show()
                            ...
                            val editNome = dialogView.findViewById<EditText>(R.id.editNomeDialog)
                            val editIdade = dialogView.findViewById<EditText>(R.id.editIddDialog)
                            val editLeitura = dialogView.findViewById<EditText>(R.id.editLeituraDialog)
                            val editMatematica = dialogView.findViewById<EditText>(R.id.editMatematicaDialog)
                            ...
                            btnSalvar.setOnClickListener {
                                ...
                                val idadeTexto = editIdade.text.toString().trim()
                                val leitura = editLeitura.text.toString().trim()
                                val matematica = editMatematica.text.toString().trim()

                                if (nome.isEmpty() || idadeTexto.isEmpty() || leitura.isEmpty() || matematica.isEmpty()) {
                                ...
                                crianca?.id?.let {
                                    id -> database.getReference("criancas").child(id).updateChildren(novosDados)
                                    mostrarMensagem("Criança atualizada!")
                                }
                                dialog.dismiss()
                            }
                        }
