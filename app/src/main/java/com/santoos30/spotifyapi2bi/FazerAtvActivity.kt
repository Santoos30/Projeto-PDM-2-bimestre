...
class FazerAtvActivity : AppCompatActivity() {
    ...
    private fun salvarResultado() {
        ...
        if (listaCriancas.isEmpty() || listaAtividades.isEmpty()) {
            mostrarMensagem(
                "Cadastre crianças e atividades primeiro!"
            )
            return
        }
        ...
        val acertou = limparTexto(resposta) ==
                limparTexto(atividade.respostaCorreta ?: "")

        val idResultado = database
            .getReference("resultados")
            .push()
            .key
